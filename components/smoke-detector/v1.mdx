...
summary: "Ceiling-mounted FireAngel ST-622 smoke alarm above the IOS: 85 dB sounder, sealed 10-year battery, weekly test."
---
...
![FireAngel ST-622 optical smoke alarm](assets/ST-622-hero2.png)
...
![Front face of the ST-622, showing the centre Test / Silence button](assets/ST-622-front-1.jpg)
...
TODO(łukasz): record the audible signals from the installed alarm — at least the test response, and
the low-battery beep if it can be produced safely — and save them under `static/audio/`. Then pass the
path to the player above, for example `<SmokeAlarmSignal src="/audio/st-622-test.mp3" label="Test response" />`.
No sound is played until then: the manufacturer publishes neither the tone frequency nor the pulse
timing, so a synthesised signal could not be verified against the device.
...
2. TODO(łukasz): state the FTD order of actions before leaving the room — whether the device is shut down at the [[starting-panel]], whether an emergency stop is pressed, and who is alerted over the technician intercom.
...
![Testing the ST-622: press and release the centre button](assets/ST-622-Test.jpg)
...
![Fitting the ST-622 to its base plate: three hooks into three slots, then turn clockwise](assets/ST-622-Install.jpg)
...
- [[emergency-stop]] — the emergency stops and the other safety equipment of the IOS.
