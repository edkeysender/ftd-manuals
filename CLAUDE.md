...
This repository generates FTD.aero simulator manuals from small module chunks.
...
- `modules/<id>/module.yaml` — identity of one module (id, name, category, location, owner).
- `modules/<id>/vN.md` — one chunk per software version range; front matter `applies_to` is a semver range.
...
1. **IDs** are English kebab-case, match the software module name, and are never renamed.
   A new ID needs a `module.yaml`; only the Support lead approves new IDs.
2. **One chunk = one module.** Never describe a second module inside a chunk; link to it with `[[other-id]]`.
...
4. **Links** between modules are always `[[module-id]]`. Never write a URL or a file path to another chunk.
...
7. **Images**: put files in `modules/<id>/assets/`, reference relatively; prefer SVG for panel diagrams.
...
1. Identify which module(s) it describes. Check `modules/` for an existing ID first.
...
5. Commit on a branch named `doc/<module-id>-<short-topic>` and open a PR; do not push to `main`.
